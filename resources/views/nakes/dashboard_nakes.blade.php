...
<script>
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        async function loadPosyandu() {
            ...
            document.getElementById("kelurahan_name").value = `Kelurahan ${kelurahan.nama_kelurahan}`;
            document.getElementById("kelurahan_id").value = kelurahan.id;
            document.getElementById("kelurahan_id_chart").value = kelurahan.id;

            posyanduData = kelurahan.posyandu || [];
            renderPosyanduDropdown(posyanduData);
            renderPosyanduDropdownChart(posyanduData);
        }
        ...
        function renderPosyanduDropdownChart(posyanduList = []) {
            ...
            btnAll.type = 'button';
            btnAll.className = 'dropdown-item block w-full text-center px-4 py-1 text-sm font-semibold text-[#61359C] hover:bg-gray-100';
            btnAll.textContent = 'Semua Posyandu';

            btnAll.onclick = () => {
                setDropdownLabel('posyanduFilterDropdownChart', 'Semua Posyandu', 'Pilih Posyandu');
